...
Aegis handles code review operations by routing GitHub webhooks through a high-performance background queue to a compiled state graph:
...
Integrating Aegis with your own repository requires deploying the application to a server (or keeping a local machine active with ngrok) and configuring a GitHub Webhook to send pull request events to it:

1. **Deploy Aegis**: Host the Aegis API, worker, and Redis queue on a server (e.g. EC2, DigitalOcean, Railway, or Render) so it is publicly accessible from the internet.
2. **Generate GitHub Personal Access Token (PAT)**:
   ...
   - Configure this value as `GITHUB_TOKEN` on your Aegis server.
...
4. **Configure Target Repository details**:
   - On your Aegis server, configure `GITHUB_REPO_OWNER` and `GITHUB_REPO_NAME` to match the target repository details.
5. **Open a PR**:
   - Any pull request lifecycle actions (opened, synchronized, or reopened) in that repository will now automatically trigger a webhook event. Aegis will process the diff and post the review comment back to the PR within 30 seconds.

---

## Testing Methodologies

Aegis provides two distinct methods for testing: **Offline Simulation** (verifying LLM output and LangGraph routing) and **End-to-End Live Webhook Testing** (verifying FastAPI, Redis, and GitHub integrations).
...
Within 30 seconds, Aegis will process the webhook, execute the agent graph, and post the synthesized markdown review comment on your Pull Request discussion timeline.
